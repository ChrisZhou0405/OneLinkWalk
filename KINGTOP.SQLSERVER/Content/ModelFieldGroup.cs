...
{
    public class ModelFieldGroup : KingTop.IDAL.Content.IModelFieldGroup
    {
        #region 根据传入的参数查询K_ModelFieldGroup,返回查询结果
        /// <summary>
        /// 根据传入的参数查询K_ModelFieldGroup,返回查询结果
        /// </summary>
        ...
        public DataTable GetList(string tranType, KingTop.Model.SelectParams paramsModel)
        {
            ...
            return SQLHelper.ExecuteDataSet(SQLHelper.ConnectionStringLocalTransaction,
                      CommandType.StoredProcedure, "proc_K_ModelFieldGroupSel", prams).Tables[0];
        }
        #endregion

        #region 设置或者删除K_ModelFieldGroup记录
        /// <summary>
        /// 设置或者删除K_ModelFieldGroup记录
        /// </summary>
        ...
        public string ModelFieldGroupSet(string tranType, string setValue, string IDList)
        {
            string sRe = "";
            try
            {
                ...
                SQLHelper.ExecuteNonQuery(SQLHelper.ConnectionStringLocalTransaction,
                          CommandType.StoredProcedure, "proc_K_ModelFieldGroupSet", prams);
                sRe = "1";
            }
            catch (Exception ex)
            {
        ...
        #region 增、改K_ModelFieldGroup表
        /// <summary>
        /// 增、改K_ModelFieldGroup表
        /// </summary>
        /// <param Name="tranType">操作类型，NEW=增，EDIT=改</param>
        /// <param Name="paramsModel"></param>
        /// <returns>返回大于0的数字操作正常，返回非数字表示操作错误，返回的是系统错误说明</returns>
        public string Save(string tranType, KingTop.Model.Content.ModelFieldGroup paramsModel)
        {
            string isOk = "";
            try
            {
                ...
                string cmdText = "proc_K_ModelFieldGroupSave";
