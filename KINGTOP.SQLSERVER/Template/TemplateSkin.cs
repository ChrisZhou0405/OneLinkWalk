...
{
    public class TemplateSkin : ITemplateSkin
    {
        ...
        public string TemplateSkinSet(string tranType, string setValue, string IDList)
        {
            ...
            try
            {
                ...
                SQLHelper.ExecuteNonQuery(SQLHelper.ConnectionStringLocalTransaction,
                          CommandType.StoredProcedure, "proc_K_TemplateSkinSet", prams);
                sRe = "1";
            }
            catch (Exception ex)
            {
        ...
        /// <returns>返回大于0的数字操作正常，返回非数字表示操作错误，返回的是系统错误说明</returns>
        public string Save(string tranType, KingTop.Model.TemplateSkin paramsModel)
        {
            string isOk = "";
            try
            {
                ...
                string cmdText = "proc_K_TemplateSkinSave";

                SqlParameter[] paras = new SqlParameter[]{
                    new SqlParameter("tranType", tranType),
                    new SqlParameter("ID",paramsModel.ID),
                    new SqlParameter("ProjectID",paramsModel.ProjectID),
                    new SqlParameter("SkinName",paramsModel.SkinName),
                    new SqlParameter("Devise",paramsModel.Devise),
                    new SqlParameter("Dirct",paramsModel.Dirct),
                    new SqlParameter("IsDefault",paramsModel.IsDefault),
                    new SqlParameter("IsDel",paramsModel.IsDel),
                    new SqlParameter("DelTime",paramsModel.DelTime),
        ...
        public void PageData(KingTop.Model.Pager pager)
        {
            SqlParameter[] prams = new SqlParameter[]{
                ...
                new SqlParameter("@Table","K_TemplateSkin"),             
