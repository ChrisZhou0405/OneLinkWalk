...
namespace KingTop.Common
{
    ...
    public class SqlDmoHelper
    {
        ...
        /// </code>
        /// </example>
        public int BackupDB(string dbName, string[] bakFile, string bakSetName, string bakDescription)
        {
            int re=0;
            //bool bak1 = BackUpDb(dbName, bakFile[1], bakSetName);
            bool bak1 = BackUpDb(dbName, bakFile[0], bakSetName);
            if (bak1)
            {
            ...
            if (!bak1)
            {
                bak1 = BackUpDb(dbName, bakFile[0], bakSetName);
                if (!bak1)
                {
                    re = 3;
                }
