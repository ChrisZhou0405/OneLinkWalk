...
namespace KingTop.WEB.SysAdmin.SysManage
{
    public partial class UploadSet : AdminPage
    {
        ...
        {
            KingTop.Config.Upload.ConfigInfo = M_UploadConfig;
            if (!File.Exists(PhyFilePath))
            {
                KingTop.Config.Upload.SaveConfig(PhyFilePath);
            }
        }

        protected void BindDate()
        {
            M_UploadConfig = KingTop.Config.Upload.GetConfig(PhyFilePath);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
             if (IsHaveRightByOperCode("Edit"))
            {
                ...
                KingTop.Config.Upload.ConfigInfo = M_UploadConfig;
                bool b=KingTop.Config.Upload.SaveConfig(PhyFilePath);
                if (!b)
                {
