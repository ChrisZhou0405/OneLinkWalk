    ...
    <div class="wrap">
        ...
        <div class="l-frame-top">
            <div class="head headbg">
                <h1>
                    <img src="../images/logo1.png" width="260" height="28" /></h1>
                <div class="nav">
                    <ul class="navlist">
                        <%=strNode%>
                ...
                <div class="rhead">
                    ...
                    <div class="admin2"><a href="../logout.aspx">
                        <img src="../images/zhuxiao.jpg" width="16" height="15" align="top" />
                        注销</a> <a href="http://www.toprand.com/feedback/index.asp">
                            <img src="../images/help.jpg" width="17" height="15" align="absmiddle" />
                            帮助</a> <a href="#">
                                <img src="../images/home.jpg" width="14" height="15" align="absmiddle" />
                                网站首页</a></div>
...
<script type="text/javascript">
    $(function () {
        $(".l-tree-node-link").unbind("click").click(function () {
            var base = $(this).parents(".l-tree-node");
            var icon = $(this).next(".l-tree-node-icon");
            var treelevel = parseInt(base.attr("treelevel"));
            //关闭
            if ($(this).hasClass("l-tree-status-open")) {
                $(this).removeClass("l-tree-status-open").addClass("l-tree-status-close");
                icon.removeClass("l-tree-icon-folderopen").addClass("l-tree-icon-folder");
                setNodes(base, treelevel, false);
            }
            else//打开
            {
                $(this).removeClass("l-tree-status-close").addClass("l-tree-status-open");
                icon.addClass("l-tree-icon-folderopen").removeClass("l-tree-icon-folder");
                setNodes(base, treelevel, true);
            }
        });
        $(".l-tree-status-open").click();
    });

    function setNodes(base, treelevel, isShow) {
        var newtreelevel = treelevel + 1;
        var rows = base.nextAll(".l-tree-node");
        for (var i = 0; i < rows.length; i++) {
            var item = $(rows[i]);
            var dtreelevel = parseInt(item.attr("treelevel"));
            if (dtreelevel == newtreelevel) {
                if (isShow) {
                    item.show();
                    var count = $(item).find(".l-tree-status-open").length;
                    //配置子级
                    setNodes(item, newtreelevel, count > 0);
                }
                else {
                    item.hide();
                    //配置子级
                    setNodes(item, newtreelevel, isShow);
                }
            }
            else if (dtreelevel == treelevel) {
                break;
            }
        }
    }      
