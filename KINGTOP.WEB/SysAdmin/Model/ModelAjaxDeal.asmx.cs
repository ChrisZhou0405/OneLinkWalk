...
namespace KingTop.WEB.SysAdmin.Model
{
    ...
    public class SysField : System.Web.Services.WebService
    {
        ...
        //拖动排序
        [WebMethod]
        public void SetOrderDrag(string tableName, string idlist, string sortList)
        {
            string sql = string.Empty;
            string[] idArr = idlist.Split(',');
            string[] valueArr = sortList.Split(',');
            for (int i = 0; i < idArr.Length; i++)
            {
                sql += "update " + tableName + " Set orders=" + valueArr[i] + " Where id='" + idArr[i] + "' AND orders<>" + valueArr[i] + ";";
            }
        ...
        #region 检查文件是否存在 上传图片（文件）预览或下载时用到
        [WebMethod]
        public bool CheckFilesExists(string filesDir)
        {
            string filePath = Server.MapPath(filesDir);
        ...
        #region 删除文件 上传文件或图片时用到
        [WebMethod]
        public bool DeleteFiles(string filesDir)
        {
            string filePath = Server.MapPath(filesDir);
            bool re = true;
            if (File.Exists(filePath))
            ...
            if (File.Exists(filePath + ".gif"))  //如果存在缩略图，则删除
            {
                try
                {
