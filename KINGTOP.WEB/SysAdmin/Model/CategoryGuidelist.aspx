...
<div class="container"id="_ListInfoListTable">
  <h4>位置： <%GetPageNav(NodeCode);%></h4> <!--不能换行,否则程序会出错-->
      ...
      <div id="searchContainer">
        <ul>
        <li>按楼层</li><li><select  id="Floor" name="Floor"><option   value="0">B1</option><option   value="1">L1</option><option   value="2">L2</option><option   value="3">L3</option><option   value="4">L4</option><option   value="5">L5</option><input type="hidden" value="6" id="{#ID#}"  name="Floor{Type}" /></select></li><li>店铺名称</li><li><input   type="text" value="" style="width:150px;"  maxlength="300" name="Title" id="Title" /></li>
    ...
    <asp:PlaceHolder ID="plParseModelLinkButton" runat="server"><%--<input type="button" onclick ="CloseOrOpenSortTable(this)" value="关闭拖动排序" />--%><input type="button" onclick='backOriginalUrl(<%=originalUrl %>)' value="返回" style=" display:<%=originalUrlDisplay%>;"/><%if(!string.Equals(ctrManageList.IsArchiving,"1") && !string.Equals(ctrManageList.IsDel,"1")){%><input type="button" value="添加" Href="CategoryGuideedit.aspx?action=add"  EditUrlParam="" id="btnNew" /><%}%><%if(!string.Equals(ctrManageList.IsDel,"1")){%><input type="submit" value="删除" onclick="return confirmSetAction(this,'{e}{(IsDel)(1)}','确定删除选定记录至回收站？');"  runat="server" id="btnDelete" /><%}%><%if(ctrManageList.IsAllowFlow && !string.Equals(ctrManageList.IsArchiving,"1") && !string.Equals(ctrManageList.IsDel,"1")){%><input type="submit"  onclick="return setAction('HQB_PastFlowCheck');" value="通过审核"  runat="server"  id="btnCheck" /><%}%><%if(ctrManageList.IsAllowFlow && !string.Equals(ctrManageList.IsArchiving,"1") && !string.Equals(ctrManageList.IsDel,"1")){%><input type="submit"  onclick="return setAction('HQB_CancelFlowCheck');" value="取消审核"  runat="server"  id="btnCancelCheck" /><%}%></asp:PlaceHolder>
        ...
        <td  style="width:45px; text-align:center;"><input type="checkbox" name="SlectAll" id="SlectAll" /></td><td  style="width:100px;" align=center><a href="javascript:sort('K_U_CategoryGuide.Title','3')">店铺名称</a></td><td  style="width:100px;text-align:left;">对应类别</td><td  style="width:100px;text-align:left;">位置</td><td  style="width:60px;text-align:left;">按字母</td><td  style="width:60px;text-align:left;">按楼层</td><td  style="width:100px;text-align:left;">是否为推荐店铺</td><td  style="width:100px;text-align:left;">是否为本季热店</td><td  style="width:100px;text-align:left;">审核状态</td><td  style="width:100px;" align=center><a href="javascript:sort('K_U_CategoryGuide.Orders','3')">排序</a></td><td style="width:80px;" >操作</td>
        </tr>
        <asp:Repeater ID="rptListInfo" runat="server">
        <ItemTemplate>
        <%#GetSortList(Eval("orders").ToString ()) %>
         <tr class="listInfotr"><span id='Title_<%#Eval("ID") %>' style='display:none'><%#Eval("Title") %></span><td style="width:45px; text-align:center;"><input type="checkbox" name="_chkID" value="<%#Eval("ID") %>" /></td><td isClip="1"  style="text-align:left;width:100px;"><%#Eval("Title") %></td><td  style="text-align:left;width:100px;"><%#Eval("type") %></td><td isClip="1"  style="text-align:left;width:100px;"><%#Eval("ShopNo") %></td><td  style="text-align:left;width:60px;"><%#ctrManageList.ParseFieldValueToText("100011444931154",Eval("LetterQuery") )%></td><td  style="text-align:left;width:60px;"><%#ctrManageList.ParseFieldValueToText("100011451892536",Eval("Floor") )%></td><td  style="text-align:left;width:100px;"><%#ctrManageList.ParseFieldValueToText("100011446354323",Eval("isrecommend") )%></td><td  style="text-align:left;width:100px;"><%#ctrManageList.ParseFieldValueToText("100011447069759",Eval("IsHot") )%></td><td  style="text-align:left;width:100px;"><%#ctrManageList.ParseFieldValueToText("100011439615262",Eval("FlowState") )%></td><td  id="HQB_Orders_<%#Eval("ID") %>" class="dragOrders" style="width:100px;" align=center>
   ...
   <asp:HiddenField ID="hdnNotSearchField" runat="server" value="" /><asp:HiddenField ID="hdnBackDeliverUrlParam" runat="server" value="NodeCode"  /> <asp:HiddenField ID="hdnModelID" Value="100000006215625" runat="server" /><asp:HiddenField ID="hdnTableName" Value="K_U_CategoryGuide" runat="server" />  <asp:HiddenField ID="hdnDeliverAndSearchUrlParam" runat="server"  Value=""/> <input type="hidden" value="" id="HQB_Model_DeliverUrlParam" /><asp:HiddenField ID="hdnCustomCol" Value="" runat="server" />   <asp:HiddenField ID="hdnForignTableCol" value="[K_U_Category]|K_U_CategoryGuide.type|Title|ID" runat="server" /><asp:HiddenField ID="hdnShowCol" value="K_U_CategoryGuide.ID,K_U_CategoryGuide.FlowState,K_U_CategoryGuide.Title,K_U_CategoryGuide.type,K_U_CategoryGuide.ShopNo,K_U_CategoryGuide.LetterQuery,K_U_CategoryGuide.Floor,K_U_CategoryGuide.isrecommend,K_U_CategoryGuide.IsHot,K_U_CategoryGuide.Orders" runat="server" />
