    ...
    <div class="container">
        ...
        <ul style=" list-style:none; height:25px;">
            ...
            <li style="width:58%; float:left; text-align:right;">
                ...
                <%--暂时关闭该功能<input type="button" value="推荐区域" onclick="openframe({url:'RecommendAreaSet.aspx?ID=<%=ModelID %>&NodeCode=<%=NodeCode %>&ModelName=<%=ModelName %>',width:'600', height:'300'})" />--%>
        ...
        <ul class="ulheader">
            ...
            <li style="width:80px;"><a href="javascript:sort('Orders')">排序</a></li>
        ...
        <asp:Repeater ID="rptModelField" runat="server">
            <ItemTemplate>
                <ul class="ulheader ulbody" onmouseover="$(this).addClass('rowSelected')" onmouseout="$(this).removeClass('rowSelected')">
                    ...
                    <li style="width:80px; text-align:center;" id="HQB_Orders_<%#Eval("ID") %>"><div style=" display:none;" ><img src="../image/loading.gif"/></div><span><input type="text" style="width: 50px;" value="<%#Eval("Orders") %>" onblur="setOrders('K_ModelField', '<%#Eval("ID") %>', this.value)" /></span></li>
                    <li  style="width:100px;">
                        <%if (IsDel == "0" || IsDel == "")
                          { %>
                        <input type="button" value="修改" class="btn"　id="btnEdit"　runat="server" BtnType="edit"  RecordID='<%#Eval("ID") %>' />
                        <asp:Button ID="btnDelete" runat="server" CssClass="btn"　OnCommand="FieldManage_Del" CommandName="DELONE" Enabled='<%#!Convert.ToBoolean(Eval("IsSystemFiierd")) %>' CommandArgument='<%#Eval("ID") %>'  OnClientClick="return reConfirm(this,'确定要删除这个字段至回收站吗?');" RecordID='<%#Eval("ID") %>' Text="删除"></asp:Button>
                            <%}
                          else
                          { %>
                          <span>
                        <asp:Button ID="btnRevert" runat="server" CssClass="btn" OnCommand="FieldManage_Del" CommandName="REVERT" CommandArgument='<%#Eval("ID") %>' OnClientClick='return confirm("确定还原这个字段吗?")' Text="还原"></asp:Button>
                           <asp:Button runat="server" CssClass="btn" OnCommand="FieldManage_Del" CommandName="DELDP" Enabled='<%#!Convert.ToBoolean(Eval("IsSystemFiierd")) %>' CommandArgument='<%#Eval("ID") %>' OnClientClick='return confirm("确定要彻底删除这个字段吗?")' Text="删除"></asp:Button>
                            </span>
                             <%} %>
                    </li>
                </ul>
