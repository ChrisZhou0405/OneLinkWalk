...
namespace KingTop.Web.Admin
{
    public class AdminPage : System.Web.UI.Page
    {
        ...
        protected string FAction { get { return m_FAction; } set { m_FAction = value; } }
        //初始化
        protected override void OnInit(EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(FAction)) FAction = Request["Action"];
            if (!string.IsNullOrEmpty(FAction)) FAction = FAction.ToLower();
        }
        #endregion
        ...
        /// 2012-09-14
        /// </summary>
        /// <param name="s">返回内容</param>
        protected virtual void ShowText(string s)
        {
            ShowText(s, Encoding.UTF8);
        }
        protected virtual void ShowText(string s, Encoding enc)
        {
            Response.Clear();
        ...
        public string StrPageParams
        {
            get
            {
                ...
                string strPageParams = KingTop.Common.Utils.GetCookie(SystemConst.COOKIES_PAGE_KEY, strCookieKey);
                if (string.IsNullOrEmpty(strPageParams))
                {
                    strCookieKey = sessionid + Utils.ReqUrlParameter("NodeCode");
                    strPageParams = KingTop.Common.Utils.GetCookie(SystemConst.COOKIES_PAGE_KEY, strCookieKey);
                }
                if (string.IsNullOrEmpty(strPageParams))
                {
        ...
        public void SetRight(Page p, Repeater rpt, out string ActionList)
        {
            KingTop.BLL.SysManage.RightTool RightToo = new KingTop.BLL.SysManage.RightTool();

            bool b = RightToo.SetRight(NodeCode, GetLoginAccountId(), GetLoginUserGroupCode(), p, rpt, out ActionList);
            if (b == false)
            {
        ...
        public void SetRight(Page p, Repeater rpt)
        {
            KingTop.BLL.SysManage.RightTool RightToo = new KingTop.BLL.SysManage.RightTool();
            string actList;
            bool b = RightToo.SetRight(NodeCode, GetLoginAccountId(), GetLoginUserGroupCode(), p, rpt, out actList);
            if (b == false)
            {
                Response.Redirect("../NoRight.aspx");
            }
        ...
        public string GetUploadImgUrl()
        {
            ...
            {
                reUrl = GetVirtualPath + uploadobj.ImageSavePath + "/Images";
                reUrl = reUrl.Replace("//", "/");
            }
            else
            {
        ...
        public string GetUploadFileUrl()
        {
            ...
            {
                reUrl = GetVirtualPath + uploadobj.FileSavePath + "/Files";
                reUrl = reUrl.Replace("//", "/");
            }
            else
            {
        ...
        public string GetUploadMediaUrl()
        {
            ...
            {
                reUrl = GetVirtualPath + uploadobj.MediaSavePath + "/Medias";
                reUrl = reUrl.Replace("//", "/");
            }
            else
            {
        ...
        {
            if (string.IsNullOrEmpty(nodeCode) || IsModelList)
                return;
        ...
        public static string GetPageNavStr(string nodeCode)
        {
            ...
            while (i != nodeCode.Length)
            {
                ...
                if (dr.Length > 0)
                {
                    ...
                    result += "<a href=\"#\">" + dr[0]["NodeName"].ToString() + "</a>";
                }
            }
            return result;
        }
        ...
        public bool IsModelList = false;
        #endregion

        #region 根据审核状态ID获得审核名称 适应于一级审核
        public string GetCheckName(string flowState)
        {
            string name = string.Empty; ;
            switch (flowState)
            {
                case "0":
                    name = "<font color=green>审核不通过</font>";
                    break;
                case "99":
                    name = "<font color=red>审核通过</font>";
                    break;
                case "3":
                    name = "未审核";
                    break;
        ...
        #region 双引号和单引号转义
        public string FormatInputValue(string value)
        {
