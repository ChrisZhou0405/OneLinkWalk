...
var Albums = function() {
    ...
    save = function(xcid) {
        ...
        $("#" + xcid + "AlbumsContainer").find("li").each(function() {
    ...
    addImg = function(imgTitle, imgUrl, thumbUrl, xcid) {
        var htmlCode;

        if (thumbUrl == null) {
            htmlCode = "<li RsID=\"" + rsID + "\" fieldName=\"" + fieldName + "\" rootPath=\"" + rootPath + "\" img=\"" + imgUrl + "\" id=\"" + fieldName + "_Lst_" + counter.toString() + "\">";
        } else {
        ...
        //htmlCode += " </table>";
        htmlCode += "<img src=\"" + rootUrl + imgUrl + "\" width=\"130\" height=\"90\" /><span><a href='javascript:' onclick=\"AlbumsDel('" + fieldName + "_Lst_" + counter.toString() + "')\"><img src='../images/close.png'></a></span>";
        htmlCode += "<div><input type=\"text\" value=\"" + imgTitle + "\" style=\"border:1px solid #CCCCCC;width:125px\"></div>";
        htmlCode += " </li>";
        if (xcid != null && xcid != undefined && xcid != "undefined") {
            $("#" + xcid + "AlbumsContainer").append(htmlCode);
        }
        else {
    ...
    this.sync = function(xcid) {

        if (currentImgUrl != albumsNewURL.value) {
            currentImgUrl = albumsNewURL.value;

            if (thumbAlbumsNewURL != null && thumbAlbumsNewURL != undefined) {
                addImg(albumsNewTitle.value, currentImgUrl, thumbAlbumsNewURL.value, xcid);
            } else {
                addImg(albumsNewTitle.value, currentImgUrl, null, xcid);
            }
        }
    }

    this.init = function(xcid) {
        var imgTitle, imgUrl;

        $(albumsNewBtn).click(function() {
            InputImages(uploadParam.FormName, uploadParam.ElementName, uploadParam.ControlType, uploadParam.ExtType, uploadParam.MaxSize, uploadParam.GetSizeControl, uploadParam.ThumbWidth, uploadParam.ThumbHeight, uploadParam.IsMult, uploadParam.BestWidth, uploadParam.BestHeight);
        });

        if (hasTumb && thumbAlbumsNewBtn != null) {
            $(thumbAlbumsNewBtn).click(function() {
                InputImages(uploadParam.FormName, uploadParam.ThumbElementName, uploadParam.ControlType, uploadParam.ExtType, uploadParam.MaxSize, uploadParam.GetSizeControl, uploadParam.ThumbWidth, uploadParam.ThumbHeight, uploadParam.IsMult, uploadParam.BestWidth, uploadParam.BestHeight);
            });
        }
        ...
        $("#theForm").submit(function() { save(xcid); });
    }
}
