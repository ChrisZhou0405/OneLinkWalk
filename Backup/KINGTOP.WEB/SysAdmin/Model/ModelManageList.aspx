...
<body>
    ...
    <div class="container" id="ModelManageInfoList">
        ...
        <div class="function">
            ...
            <%--<asp:Button ID="btn_Import" runat="server" Text ="导入" />--%>
            <asp:Button ID="btn_Export" runat="server" Text ="导出" OnClick ="btnExport_Click" />
        ...
        <asp:Repeater ID="rptModelManage" runat="server">
            <ItemTemplate>
                <ul class="ulheader ulbody" onmouseover="$(this).addClass('rowSelected')" onmouseout="$(this).removeClass('rowSelected')">
                    ...
                    <li style="width: 100px; text-align: center;" id="HQB_Orders_<%#Eval("ID") %>">
                    ...
                    <li style="width: 440px">
                        <%if (IsDel == "")
                          { %>
                        <input type="button" value="字段" class="btn" onclick="return lookFieldList('FieldManageList.aspx?NodeCode=<%=NodeCode %>&ModelName=<%#Eval("Title") %>&modelid=<%#Eval("id") %>&tableName=<%#Eval("TableName") %>&IsSub=<%=IsSub %>','<%#Eval("IsEnable") %>')" />
                        <input type="button" value="修改" class="btn" id="btnEdit" runat="server" btntype="edit" recordid='<%#Eval("ID") %>' />
                        <asp:Button ID="btnDelete" CssClass="btn" runat="server" OnCommand="ModelManage_Del" CommandName="del"
                            CommandArgument='<%#Eval("ID") %>' OnClientClick="return reConfirm(this,'确定要删除这个模型至回收站吗?');"
                            RecordID='<%#Eval("ID") %>' Text="删除"></asp:Button>
                        <input type="button" value="参数设置" class="btn" onclick="location.href='ModelListConfig.aspx?ID=<%#Eval("ID") %>&RefUrl=ModelManageList.aspx&NodeCode=<%=NodeCode %>&ModelName=<%# Eval("Title")%>&IsSub=<%=IsSub %>'" />
                        ...
                        <asp:Button ID="btnCreateModel" CssClass="btn" RecordID='<%#Eval("ID") %>' OnClientClick="setLogTitle(this)"
                        ...
                        <%}
                          ...
                          { %>
                        <span>
                            <asp:Button ID="btnRevert" CssClass="btn" runat="server" OnCommand="ModelManage_Del" CommandName="revert"
                                CommandArgument='<%#Eval("ID") %>' OnClientClick='return confirm("确定还原这个模型吗?")'
                                Text="还原"></asp:Button>
                            <asp:Button runat="server" CssClass="btn" OnCommand="ModelManage_Del" CommandName="deldp" ToolTip='<%#Eval("TableName")%>'
                                CommandArgument='<%#Eval("ID") %>' OnClientClick='return confirm("确定要彻底删除这个模型吗?")'
                                Text="删除"></asp:Button>
                        </span>
                        <%} %>
                        <input type="button" value="复制" class="btn" id="btnCopy" onclick="return lookFieldList('ModelMangeCopy.aspx?NodeCode=<%=NodeCode %>&ModelName=<%#Eval("Title") %>&modelid=<%#Eval("id") %>&tableName=<%#Eval("TableName") %>&IsSub=<%=IsSub %>','<%#Eval("IsEnable") %>')" btntype="edit" recordid='<%#Eval("ID") %>' />
                        <input type="button" value="生成代码" class="btn" id="btnCode" onclick="return lookFieldList('ModelManageCode.aspx?NodeCode=<%=NodeCode %>&ModelName=<%#Eval("Title") %>&modelid=<%#Eval("id") %>&tableName=<%#Eval("TableName") %>&IsSub=<%=IsSub %>','<%#Eval("IsEnable") %>')"  btntype="edit" recordid='<%#Eval("ID") %>' />
                    </li>
                </ul>
            </ItemTemplate>
        </asp:Repeater>
