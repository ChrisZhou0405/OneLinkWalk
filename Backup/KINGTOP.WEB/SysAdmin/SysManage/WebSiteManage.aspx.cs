...
namespace KingTop.WEB.SysAdmin.SysManage
{
    public partial class WebSiteManage : AdminPage
    {
        ...
        public string GetUpdateMenu(string siteId)
        {
            string re = "";
            string sql = "select top 1 NodeCode from K_SysModuleNode Where WebSiteID=" + siteId + " and IsWeb=1 Order By NodeCode";
            DataTable dt = SQLHelper.GetDataSet(sql);
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                re = "<a class=\"abtn\"  href=\"NewWebSetMenuEdit.aspx?NodeCode="+NodeCode +"&parentNodeCode="+dt.Rows[0]["NodeCode"]+"\">批量修改栏目</a>";
            }
