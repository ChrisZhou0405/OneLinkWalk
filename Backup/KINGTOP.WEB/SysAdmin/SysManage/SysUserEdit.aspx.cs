...
namespace KingTop.WEB.SysAdmin.SysManage
{
    public partial class SysUserEdit : AdminPage
    {
        ...
        KingTop.Model.SysManage.Account MAccount = new KingTop.Model.SysManage.Account();
        ...
        {
            KingTop.BLL.SysManage.Account objAccount = new Account();
            DataTable dt = objAccount.GetList("GETBYNAME", Utils.getOneParams(UserName));
            if (dt.Rows.Count > 0)
            {
                if (KingTop.Common.AppCache.IsExist("UserID"))
                {
                    KingTop.Common.AppCache.Remove("UserID");
                }
                KingTop.Common.AppCache.AddCache("UserID", dt.Rows[0]["UserID"].ToString(), 120);
                return true;
            }
            else
            {
