            ...
            if (IsHaveRightByOperCode("Edit"))
            {
                ...
                if (isMainDomain)
                {
                    ...
                    if (this.RadTrue.Checked)
                    {
                        ...
                        if (!Utils.XmlUpdate(siteLoginXml, "SiteInfoManage/IsEnableManageCode", "", "1"))
                        {
                            Utils.RunJavaScript(this, "alert({msg:'站点参数设置失败，配置文件“" + siteLoginXml + "”位置不对或者没有修改权限!',title:'提示信息'})");
                            return;
                         }
                ...
                SiteParam.ConfigInfo = M_SiteParamConfig;
                string publishType = Request.Form["radPublishType"];
                M_SiteParamConfig.PublishType = publishType;
                ...
                KingTop.WEB.BasePage.PublishType = publishType;
                
                //Utils.XmlUpdate(PhyFilePath, "SiteParamConfig/PublishType", "", publishType);
                if (!b)
                {
