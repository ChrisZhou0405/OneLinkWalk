var me = editor,
    ...
    var bindClick = function() {
        ...
        for (var i = 0, head; head = heads[i++]; ) {
            head.onclick = function() {
                ...
                else if (bodyid == "local") {
                if (!document.getElementById("flash")) {
                        createFlashUploader(flashOptions, callbacks);
                    }
                }
            }
        }
    ...
    var toggleBody = function(id) {
        var bodys = ["normal", "local", "imgManager"];
        for (var i = 0, body; body = bodys[i++]; ) {
            if (body == id) {
                $G(body).style.display = "";
                //$G(body).style.zIndex = 200;
            }
            else {
                $G(body).style.display = "none";
                //$G(body).style.zIndex = 1;
            }
