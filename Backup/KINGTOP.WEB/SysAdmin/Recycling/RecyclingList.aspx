...
<div class="container">
  ...
  <p><%= Resources.Common.Name%><asp:TextBox ID="txtGroupName" runat="server"></asp:TextBox>     <asp:DropDownList ID="ddlTable" runat="server"></asp:DropDownList>
                            <asp:Button ID="btnSearch" runat="server" Text="<%$Resources:Common,Search %>"       onclick="btnKeySerach_Click" />                           
      </p>
  </div> 
  <div class="function">
  ...
  <li style="width:20%">
	<asp:Button ID="Button2" CssClass="btn" runat="server" Text="还原"   CommandArgument='<%#Eval("id").ToString()+"|"+Eval("tableName2").ToString() %>' OnCommand="btn_rese" />
	<asp:Button ID="Button1" CssClass="btn" runat="server" Text="<%$Resources:Common,Del %>" OnCommand="btn_Del" CommandArgument='<%#Eval("id").ToString()+"|"+Eval("tableName2").ToString() %>'  />    			     
   </li>
  </ul>
  </ItemTemplate>
                </asp:Repeater>
